...
class EditUser extends EditRecord
{
    ...
    protected function getHeaderActions(): array
    {
        ...
        return [
            ...
            Actions\DeleteAction::make('customAction')
                ->label('Delete')
                ->action(function ($record) {

                    $this->customFunction($record);
                })
    ...
    protected function customFunction($record)
    {
        try {


            DB::table('users')->where('id', $record->id)->delete();
            DB::table('fingerprint_models')->where('employee_id', $record->id)->delete();
            ...
            Notification::make()
            ->title('Error')
            ->body('Failed to delete the record: ' . $e->getMessage())
            ->danger()
            ->send();
        }
    }
